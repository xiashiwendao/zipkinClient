 ...
 * {@link #execute() synchronously} or {@link #enqueue(Callback)
 * asynchronously}. At any time, from any thread, you can call
 * {@linkplain #cancel()}, which might stop an in-flight request or prevent one
 * from occurring.
 *
 * <p>
 * Implementations should prepare a call such that there's little or no
 * likelihood of late runtime exceptions. For example, if the call is to get a
 * trace, the call to {@code listSpans} should propagate input errors vs delay
 * them until a call to {@linkplain #execute()} or
 ...
 * {
 * 	&#64;code
 * 	// Any translation of an input request to remote parameters should happen
 * 	// here, and any related
 ...
 *
 * <p>
 * An instance of call cannot be invoked more than once, but you can
 * {@linkplain #clone()} an instance if you need to replay the call. There is no
 * relationship between a call and a number of remote requests. For example, an
 * implementation that stores spans may make hundreds of remote requests,
 * possibly retrying on your behalf.
 *
 * <p>
 * This type owes its design to {@code retrofit2.Call}, which is nearly the
 * same, except limited to HTTP transports.
 *
 * @param <V>
 *            the success type, typically not null except when {@code V} is
 *            {@linkplain Void}.
 */
public abstract class Call<V> implements Cloneable {
	/**
	 * Returns a completed call which has the supplied value. This is useful
	 * when input parameters imply there's no call needed. For example, an empty
	 * input might always result in an empty output.
	 */
	...
	public static <T> Call<List<Object>> emptyList() {
	 ...
	 * {@code mapper} function. This is used to convert values from one type to
	 * another. For example, you could use this to convert between zipkin v1 and
	 * v2 span format.
	 *
	 * <pre>
	 * {@code
	 ...
	 *
	 * <p>
	 * This method intends to be used for chaining. That means "this" instance
	 * should be discarded in favor of the result of this method.
	 */
	 ...
	 * {@code flatMapper} function. This is used to chain two remote calls
	 * together. For example, you could use this to chain a list IDs call to a
	 * get by IDs call.
	 *
	 * <pre>
	 * {@code
	...
	/**
	 * Returns a call which can attempt to resolve an exception. This is useful
	 * when a remote call returns an error when a resource is not found.
	 *
	 * <p>
	...
	/**
	 * Invokes a request, returning a success value or propagating an error to
	 * the caller. Invoking this more than once will result in an error. To
	 * repeat a call, make a copy with {@linkplain #clone()}.
	 *
	 * <p>
	 * Eventhough this is a blocking call, implementations may honor calls to
	 * {@linkplain #cancel()} from a different thread.
	 *
	 * @return a success value. Null is unexpected, except when {@code V} is
	 *         {@linkplain Void}.
	 */
	public abstract V execute() throws IOException;

	/**
	 * Invokes a request asynchronously, signaling the {@code callback} when
	 * complete. Invoking this more than once will result in an error. To repeat
	 * a call, make a copy with {@linkplain #clone()}.
	 */
	public abstract void enqueue(Callback<V> callback);

	/**
	 * Requests to cancel this call, even if some implementations may not
	 * support it. For example, a blocking call is sometimes not cancelable.
	 */
	// Boolean isn't returned because some implementations may cancel
	// asynchronously.
	// Implementing might throw an IOException on execute or
	 ...
	 *
	 * <p>
	 * Calls can fail before being canceled, so true does always mean
	 * cancelation caused a call to fail. That said, successful cancellation
	 * does result in a failure.
	 */
	// isCanceled exists while isExecuted does not because you do not need the
	// latter to implement
	// asynchronous bindings, such as rxjava2
	public abstract boolean isCanceled();

	/**
	 * Returns a copy of this object, so you can make an identical follow-up
	 * request.
	 */
	@Override
	...
	static final class ErrorHandling<V> extends Base<V> {
		static final Object SENTINEL = new Object(); // to differentiate from
														// null
