...
/**
 * As spans are reported, they are encoded and added to a pending queue. The
 * task of sending spans happens on a separate thread which calls
 * {@link #flush()}. By doing so, callers are protected from latency or
 * exceptions possible when exporting spans out of process.
 *
 * <p>
 * Spans are bundled into messages based on size in bytes or a timeout,
 * whichever happens first.
 *
 * @param <S>
 *            type of the span, usually {@link zipkin2.Span}
 */
public abstract class AsyncReporter<S> extends Component implements Reporter<S>, Flushable {
	/**
	 * Builds a json reporter for
	 * <a href="http://zipkin.io/zipkin-api/#/">Zipkin V2</a>. If http, the
	 * endpoint of the sender is usually "http://zipkinhost:9411/api/v2/spans".
	 *
	 * <p>
	 * After a certain threshold, spans are drained and
	 * {@link Sender#sendSpans(List) sent} to Zipkin collectors.
	 */
	...
	/**
	 * Like {@link #create(Sender)}, except you can configure settings such as
	 * the timeout.
	 */
	...
	/**
	 * Calling this will flush any pending spans to the transport on the current
	 * thread.
	 *
	 * <p>
	 * Note: If you set {@link Builder#messageTimeout(long, TimeUnit) message
	 * timeout} to zero, you must call this externally as otherwise spans will
	 * never be sent.
	 *
	...
	/**
	 * Shuts down the sender thread, and increments drop metrics if there were
	 * any unsent spans.
	 */
	@Override
	...
	public static final class Builder {
		...
		static int onePercentOfMemory() {
			...
			// don't overflow in the rare case 1% of memory is larger than 2
			// GiB!
		...
		/**
		 * Maximum bytes sendable per message including overhead. Defaults to,
		 * and is limited by {@link Sender#messageMaxBytes()}.
		 */
		 ...
		 *
		 * <p>
		 * Instead of sending one message at a time, spans are bundled into
		 * messages, up to {@link Sender#messageMaxBytes()}. This timeout
		 * ensures that spans are not stuck in an incomplete message.
		 *
		 * <p>
		...
		/**
		 * How long to block for in-flight spans to send out-of-process on
		 * close. Default 1 second
		 */
		...
		/**
		 * Builds an async reporter that encodes zipkin spans as they are
		 * reported.
		 */
		...
		/**
		 * Builds an async reporter that encodes arbitrary spans as they are
		 * reported.
		 */
		public <S> AsyncReporter<S> build(BytesEncoder<S> encoder) {
			...
			if (encoder.encoding() != sender.encoding()) {
				throw new IllegalArgumentException(
						String.format("Encoder doesn't match Sender: %s %s", encoder.encoding(), sender.encoding()));
			}

			final BoundedAsyncReporter<S> result = new BoundedAsyncReporter<S>(this, encoder);

			if (messageTimeoutNanos > 0) { // Start a thread that flushes the
											// queue in a loop.
				final BufferNextMessage<S> consumer = BufferNextMessage.create(sender, messageMaxBytes,
						messageTimeoutNanos);
					...
					public void run() {
						try {
							...
							if (count > 0) {
								...
								BoundedAsyncReporter.logger
										.warning("Dropped " + count + " spans due to AsyncReporter.close()");
							}
	...
	static final class BoundedAsyncReporter<S> extends AsyncReporter<S> {
		...
		public void report(S next) {
			...
			// don't enqueue something larger than we can drain
					messageSizeOfNextSpan > messageMaxBytes || !pending.offer(next, nextSizeInBytes)) {
		...
		public final void flush() {
			flush((BufferNextMessage<S>) BufferNextMessage.create(sender, messageMaxBytes, 0));
		}
		...
		void flush(BufferNextMessage<S> bundler) {
			...
			// record after flushing reduces the amount of gauge events vs on
			// doing this on report
			...
			// Signal that we are about to send a message of a known size in
			// bytes
			metrics.incrementMessages();
			metrics.incrementMessageBytes(bundler.sizeInBytes());

			// Create the next message. Since we are outside the lock shared
			// with writers, we can encode
			...
			bundler.drain(new SpanWithSizeConsumer<S>() {
				...
				public boolean offer(S next, int nextSizeInBytes) {
					nextMessage.add(encoder.encode(next)); // speculatively add
															// to the pending
															// message
					if (sender.messageSizeInBytes(nextMessage) > messageMaxBytes) {
						// if we overran the message size, remove the encoded
						// message.
		...
		private void handException(final ArrayList<byte[]> nextMessage, Throwable t) {
			...
			if (logger.isLoggable(FINE)) {
				logger.log(FINE, format("Dropped %s spans due to %s(%s)", count, t.getClass().getSimpleName(),
