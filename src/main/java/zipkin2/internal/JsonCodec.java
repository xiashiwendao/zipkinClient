...
/**
 * This explicitly constructs instances of model classes via manual parsing for
 * a number of reasons.
 *
 * <ul>
 * <li>Eliminates the need to keep separate model classes for thrift vs
 * json</li>
 ...
 *
 * <p>
 * There is the up-front cost of creating this, and maintenance of this to
 * consider. However, this should be easy to justify as these objects don't
 * change much at all.
 */
public final class JsonCodec {
	...
	public static @Nullable <T> T readOne(JsonReaderAdapter<T> adapter, byte[] bytes) {
		List<T> out = new ArrayList<T>(1); // TODO: could make single-element
											// list w/o array
		if (!read(adapter, bytes, out))
			return null;
		return out.get(0);
	}
	...
	static <T> int sizeInBytes(Buffer.Writer<T> writer, List<T> value) {
		...
		int sizeInBytes = 2; // []
		if (length > 1)
			sizeInBytes += length - 1; // comma to join elements
	...
	public static <T> byte[] write(Buffer.Writer<T> writer, T value) {
		...
		try {
			...
			// Don't use value directly in the message, as its toString might be
			// implemented using this
			// method. If that's the case, we'd stack overflow. Instead, emit
			// what we've written so far.
			String message = format("Bug found using %s to write %s as json. Wrote %s/%s bytes: %s",
					writer.getClass().getSimpleName().replace("AutoValue_", ""), value.getClass().getSimpleName(),
					lengthWritten, bytes.length, written);
