...
// @Immutable
public final class Endpoint implements Serializable { // for Spark and Flink
														// jobs
	 ...
	 *
	 * <p>
	 * Prefer using the {@link #ipv4()} field for mapped addresses.
	 */
	public String ipv6() {
		return ipv6;
	}
	...
	public static final class Builder {
		...
		public Builder serviceName(String serviceName) {
			this.serviceName = serviceName == null || serviceName.isEmpty() ? null
					: serviceName.toLowerCase(Locale.ROOT);
			return this;
		}

		/** Chaining variant of {@link #parseIp(InetAddress)} */
		public Builder ip(InetAddress addr) {
			parseIp(addr);
			return this;
		}

		/**
		 * Returns true if {@link Endpoint#ipv4()} or {@link Endpoint#ipv6()}
		...
		public Builder ip(String ipString) {
			parseIp(ipString);
			return this;
		}
		 ...
		 */
		public final boolean parseIp(String ipString) {
		 ...
		 *
		 * @param port
		 *            port associated with the endpoint. zero coerces to null
		 *            (unknown)
		 * @see Endpoint#port()
		 */
	...
	static String parseEmbeddedIPv4(byte[] ipv6) {
		for (int i = 0; i < 10; i++) { // Embedded IPv4 addresses start with
										// unset 80 bits
			if (ipv6[i] != 0)
				return null;
		}
		...
		if (flag != 0 && flag != -1)
			return null; // IPv4-Compatible or IPv4-Mapped
	...
	enum IpFamily {
		Unknown, IPv4, IPv4Embedded, IPv6
	}

	/**
	 * Adapted from code in
	 * {@code com.google.common.net.InetAddresses.ipStringToBytes}. This version
	 * separates detection from parsing and checks more carefully about embedded
	 * addresses.
	 */
	static IpFamily detectFamily(String ipString) {
		...
		for (int i = 0, length = ipString.length(); i < length; i++) {
			...
			if (c == '.') {
				...
				if (hasDot)
					return IpFamily.Unknown; // Colons must not appear after
												// dots.
				hasColon = true;
			} else if (notHex(c)) {
				return IpFamily.Unknown; // Everything else must be a decimal or
											// hex digit.
			}
		}
		...
		if (hasColon) {
			if (hasDot) {
				...
				if (lastColon == 1 && ipString.charAt(0) == ':') {// compressed
																	// like
																	// ::1.2.3.4
	...
	private static boolean isValidIpV4Address(String ip, int from, int toExcluded) {
		...
		int i;
		return len <= 15 && len >= 7 && (i = ip.indexOf('.', from + 1)) > 0 && isValidIpV4Word(ip, from, i)
	...
	private static boolean isValidIpV4Word(CharSequence word, int from, int toExclusive) {
		...
		if (len == 3) {
			return (c1 = word.charAt(from + 1)) >= '0' && (c2 = word.charAt(from + 2)) >= '0'
					&& ((c0 <= '1' && c1 <= '9' && c2 <= '9')
	...
	public String toString() {
		return "Endpoint{" + "serviceName=" + serviceName + ", " + "ipv4=" + ipv4 + ", " + "ipv6=" + ipv6 + ", "
				+ "port=" + port + "}";
	}
	...
	public boolean equals(Object o) {
		...
		return ((serviceName == null) ? (that.serviceName == null) : serviceName.equals(that.serviceName))
	...
	// As this is an immutable object (no default constructor), defer to a
	// serialization proxy.
